...
from utils.data_reader import load_dataset, load_dataset_epitome
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if config.empathy_label:
            item['er_label'] = self.data["er_label"][index]
            item['ip_label'] = self.data["ip_label"][index]
            item['ex_label'] = self.data["ex_label"][index]
...
def collate_fn(batch_data):
    ...
    if config.empathy_label:
        d["er_label"] = torch.LongTensor(item_info['er_label']).to(config.device)  # (bsz,)
        d["ip_label"] = torch.LongTensor(item_info['ip_label']).to(config.device)  # (bsz,)
        d["ex_label"] = torch.LongTensor(item_info['ex_label']).to(config.device)  # (bsz,)
...
def prepare_data_seq(batch_size=32, adver_train=False, gen_data=False):
    ...
    print("Vocab  {} ".format(vocab.n_words))
