...
def evaluate(model, data, ty='valid', max_dec_step=30, adver_train=False):
    outputs = open("Predictions/response.txt", "w", encoding="utf-8")
    model.eval()
    model.cuda()
    ...
    P, R, F1 = bert_score.score(hyp_g, ref, lang='en', verbose=True)
    bsp, bsr, bsf = torch.mean(P).item(), torch.mean(R).item(), torch.mean(F1).item()

    if ty != 'valid':
        print("EVAL: {}, Loss: {:.4f}, BLEU: {:.4f}, PPL: {:.4f}, Accuracy: {:.4f}, BSP: {:.4f}, BSR: {:.4f}, BSF: {:.4f}".format(ty, loss, bleu_score_g, math.exp(loss), acc, bsp, bsr, bsf))
        return loss, bleu_score_g, math.exp(loss), bce, acc, bsp, bsr, bsf
    else:
        print("EVAL: {}, Loss: {:.4f}, BLEU: {:.4f}, PPL: {:.4f}, Accuracy: {:.4f}".format(ty, loss, bleu_score_g, math.exp(loss), acc))
        return loss, bleu_score_g, math.exp(loss), bce, acc
